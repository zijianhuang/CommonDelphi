...
// Modify by Zijian. Though I can removed IDE generated codes not to create COM object.
// it will make sence that the COM Server object is not created at all.
...
//  if FStartMode in [smRegServer, smUnregServer] then Halt; // silly, memory leak likely , should left the app clean up
...
begin
  if FTypeLib <> nil then
    if Register then
      RegisterTypeLibrary(FTypeLib, GetModuleFileName)
      else
...
AutomationTerminateProc;
  ...
  InitProc := SaveInitProc; // make sure Application.iniitialize work ok
end;
