...
uses
  ...
  ToolWin, TntComCtrls, TntMenus;

type
  TTntMenuBar = class(TTntToolBar)
  private
    FMenu: TTntMainMenu;
...
procedure Register;
begin
  RegisterComponents('Samples', [TTntMenuBar]);
end;

{ TTntMenuBar }

constructor TTntMenuBar.Create(AOwner: TComponent);
begin
...
procedure TTntMenuBar.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
end;

procedure TTntMenuBar.SetMenu(const Value: TTntMainMenu);
var
  i: Integer;
...
begin
  ...
  begin
    Button := TTntToolButton.Create(Self);
    try
