...
Zijian modified this file to support unicode.

Known Issues:
-----------------------------------------------------------------------------}
// $Id: JvDragDrop.pas 10630 2006-05-31 15:42:19Z ahuser $
...
uses
  ...
  Windows, Messages, ShellAPI, ActiveX, Classes, Controls, TntClasses,
  JvComponentBase;
...
type
  ...
  TJvDropEvent = procedure(Sender: TObject; Pos: TPoint; Value: TTntStringList) of object;
  ...
  private
    ...
    function GetFiles: TTntStringList;
    procedure SetAcceptDrag(Value: Boolean);
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property Files: TTntStringList read GetFiles;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jvcl.svn.sourceforge.net/svnroot/jvcl/tags/JVCL3_32/run/JvDragDrop.pas $';
    Revision: '$Revision: 10630 $';
    Date: '$Date: 2006-05-31 17:42:19 +0200 (mer., 31 mai 2006) $';
    LogPath: 'JVCL\run'
  );
{$ENDIF UNITVERSIONING}
...
begin
  ...
  try
    ...
    begin
      ...
      DragQueryFileW(Handle, I, Buffer, BufferLength);
      FFiles.Add(Buffer);
    end;
  finally
    FreeMem(Buffer);
...
function TJvDragDrop.GetFiles: TTntStringList;
begin
  Result := FFiles;
end;
